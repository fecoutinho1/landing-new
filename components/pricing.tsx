          ...
          {PLANS.map((plan, i) => (
            <div
              ...
              className={`relative rounded-2xl border transition-all duration-700 ease-out hover:-translate-y-2 ${
                ...
                plan.highlighted
                  ? "border-[var(--color-accent-orange-400)] bg-gradient-to-b from-[var(--color-accent-orange-50)] to-white md:-translate-y-4"
                  : "border-[var(--color-baltic-sea-200)] bg-white"
              }`}
              style={{
                transitionDelay: `${i === 1 ? 100 : i === 0 ? 200 : 300}ms`,
                boxShadow: plan.highlighted ? "0 0 60px -20px var(--color-accent-orange-400)" : "var(--bento-shadow)",
              }}
            >
              {plan.highlighted && (
                <div
                  className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-[var(--color-accent-orange-500)] text-white text-xs font-semibold rounded-full flex items-center gap-1 transition-all duration-500 ${
              ...
              <div className="p-6 lg:p-8">
                <h3 className="text-lg font-semibold text-[var(--color-baltic-sea-900)]">{plan.name}</h3>
                <div className="mt-4 flex items-baseline gap-1">
                  <span className="text-4xl font-bold text-[var(--color-baltic-sea-900)]">{plan.price}</span>
                  <span className="text-[var(--color-baltic-sea-500)]">{plan.period}</span>
                </div>
                <p className="mt-2 text-sm text-[var(--color-baltic-sea-500)]">{plan.description}</p>

                <Button
                  className={`w-full mt-6 rounded-full h-11 transition-all duration-300 ${
                    plan.highlighted
                      ? "bg-[var(--color-accent-orange-500)] text-white hover:bg-[var(--color-accent-orange-600)] hover:shadow-[0_0_20px_-5px_var(--color-accent-orange-500)]"
                      : "bg-[var(--color-baltic-sea-100)] text-[var(--color-baltic-sea-800)] hover:bg-[var(--color-baltic-sea-200)]"
                  }`}
                >
                  {plan.cta}
                </Button>
                ...
                <ul className="mt-8 space-y-3">
                  {plan.features.map((feature, fi) => (
                    ...
                    >
                      <Check
                        weight="bold"
                        className={`h-5 w-5 shrink-0 ${plan.highlighted ? "text-[var(--color-accent-orange-500)]" : "text-[var(--color-baltic-sea-400)]"}`}
                      />
                      <span className="text-[var(--color-baltic-sea-600)]">{feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
